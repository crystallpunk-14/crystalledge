...
public sealed partial class CEClientWeaponSystem
{
    ...
    /// <summary>
    /// Does all of the melee effects for a player that are predicted,
    /// i.e. character lunge and weapon arc animation.
    /// Follows the MeleeWeaponSystem.Effects.cs pattern, reading animation
    /// parameters from <see cref="CEWeaponComponent"/> and <see cref="CEAttackActionPrototype"/>.
    /// </summary>
        ...
        bool predicted = true)
    {
        ...
        if (arcComponent.Animation != WeaponArcAnimation.None
            && TryComp(weapon, out CEWeaponComponent? weaponComp))
        {
            // Copy the weapon sprite onto the arc entity.
            if (user != weapon
                && TryComp(weapon, out SpriteComponent? weaponSprite))
            {
                _sprite.CopySprite((weapon, weaponSprite), (animationUid, sprite));
            }

            if (weaponComp.SwingLeft)
                angle *= -1;

            // Resolve the attack action prototype that matches this animation
            // to get AttackRate and AnimationOffset.
            foreach (var (_, protoId) in weaponComp.Attacks)
            {
                if (_proto.TryIndex(protoId, out var action) && action.Animation == animation)
                {
        ...
        switch (arcComponent.Animation)
        {
            ...
            case WeaponArcAnimation.None:
                ...
                var newLocalPos =
                    Vector2.Transform(worldPos, TransformSystem.GetInvWorldMatrix(xform.ParentUid));
                TransformSystem.SetLocalPositionNoLerp(animationUid, newLocalPos, xform);
                if (arcComponent.Fadeout)
                    _animation.Play(animationUid,
                        GetFadeAnimation(sprite, 0f, 0.15f),
                        FadeAnimationKey);
                break;
        }
    }
                ...
                new AnimationTrackComponentProperty()
                {
                    ...
                    KeyFrames =
                    {
                        new AnimationTrackProperty.KeyFrame(
                            Angle.Lerp(startRotation, endRotation, 0.0f), length * 0.0f),
                        new AnimationTrackProperty.KeyFrame(
                            Angle.Lerp(startRotation, endRotation, 0.5f), length * 0.10f),
                        new AnimationTrackProperty.KeyFrame(
                            Angle.Lerp(startRotation, endRotation, 1.0f), length * 0.15f),
                        new AnimationTrackProperty.KeyFrame(
                            Angle.Lerp(startRotation, endRotation, 0.9f), length * 0.20f),
                        new AnimationTrackProperty.KeyFrame(
                            Angle.Lerp(startRotation, endRotation, 0.80f), length * 0.6f,
                            Easings.OutQuart),
                    },
                },
                new AnimationTrackComponentProperty()
        ...
        return new Animation()
        {
            ...
            AnimationTracks =
            {
                new AnimationTrackComponentProperty()
                {
                    ...
                    {
                        new AnimationTrackProperty.KeyFrame(
                            Vector2.Lerp(startOffset, endOffset, 0f), length * 0f),
                        new AnimationTrackProperty.KeyFrame(
                            Vector2.Lerp(startOffset, endOffset, 0.65f), length * 0.10f),
                        new AnimationTrackProperty.KeyFrame(
                            Vector2.Lerp(startOffset, endOffset, 1f), length * 0.20f),
                        new AnimationTrackProperty.KeyFrame(
                            Vector2.Lerp(startOffset, endOffset, 0.9f), length * 0.30f),
                        new AnimationTrackProperty.KeyFrame(
                            Vector2.Lerp(startOffset, endOffset, 0.7f), length * 0.60f,
                            Easings.OutQuart),
                    },
                },
            },
            ...
            AnimationTracks =
            {
                new AnimationTrackComponentProperty()
                {
                    ...
                    KeyFrames =
                    {
                        ...
                        new AnimationTrackProperty.KeyFrame(
                            direction.Normalized() * 0.15f, length * 0.4f),
    ...
    private void UpdateEffects()
    {
        ...
        while (query.MoveNext(out var uid, out var trackComp, out var xform))
        {
            if (trackComp.User == null || Deleted(trackComp.User))
                continue;

            var targetPos = TransformSystem.GetWorldPosition(trackComp.User.Value);

            if (trackComp.Offset != Vector2.Zero)
            {
                var entRotation = TransformSystem.GetWorldRotation(xform);
                targetPos += entRotation.RotateVec(trackComp.Offset);
            }
