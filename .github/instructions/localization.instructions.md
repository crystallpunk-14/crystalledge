...
- Локализация EntityPrototype, а именно name, description, suffix, происходит отдельным маршрутом. Названия на английском описываются прямо внутри сущности, а локализация на русский потом отдельно через скрипт. Это значит, русская локализация нужна для всего КРОМЕ названий, описаний  и суффиксов сущностей.
